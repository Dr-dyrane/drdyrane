...
| Hierarchy | Decorative identity elements must not displace primary task content | Orb remains hero only in consultation; non-consult pages reclaim vertical space and start with task controls | `src/features/consultation/StepRenderer.tsx`, `src/features/history/HistoryView.tsx`, `src/features/drug/DrugProtocolsView.tsx`, `src/features/diagnostics/DiagnosticReviewView.tsx`, `src/features/about/AboutView.tsx` | Implemented |
...
| Navigation | Tab bars should provide direct top-level destination switching | Bottom pill now keeps fixed top-level tabs (Consult, History, Rx, Scan) to prevent destination reshuffling between pages | `src/components/layout/BottomNav.tsx` | Implemented |
| Menus | Secondary actions should remain contextual to current surface | Detached droplet FAB + `More` control keep page-specific actions accessible without crowding tab destinations | `src/components/layout/BottomNav.tsx` | Implemented |
